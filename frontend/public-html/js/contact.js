...
async function loadDeps() {
    ...
    let strs = [];
    strs.push(`<option hidden disabled selected value="0"> -- выберите направление -- </option>`);
    for (let dep of deps) {
...
async function updateOptions(depId) {
    ...
    let strs = [];
    strs.push(`<option hidden disabled selected value="0"> -- выберите услугу -- </option>`);
    ...
    strs = [];
    strs.push(`<option hidden disabled selected value="0"> -- выберите доктора -- </option>`);
